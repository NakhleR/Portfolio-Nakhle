...
The birds shader is unchanged. Its overlay runs for 1200ms on a fresh document load and skips internal Inertia navigation and reduced-motion visits. Public pages use production assets only when no Vite hot file points to a running development server; do not deploy `public/hot`.
