...
onMounted(async () => {
    ...
    function animate(now: number) {
        frame = 0;
        if (document.hidden || !inViewport || reduced.matches) return;
        ...
        while (remaining > 0) {
            ...
            rotation.velocityX +=
                (((target.x - rotation.x) * 30 - rotation.velocityX * 20) / 2) *
                step;
            rotation.velocityY +=
                (((target.y - rotation.y) * 30 - rotation.velocityY * 20) / 2) *
                step;
    ...
    function updateAnimation() {
        ...
        frame = 0;
        render();
        if (!reduced.matches && inViewport && !document.hidden) {
    ...
    function mouse(event: MouseEvent) {
        if (reduced.matches || !inViewport || event.timeStamp - lastMouse < 100)
            return;
